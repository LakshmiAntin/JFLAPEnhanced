...
public class EnvironmentFrame extends JFrame {
	...
	protected void refreshTitle() {
		String title = "";
		title = DEFAULT_TITLE + " : " + getDescription();

		if((this.getEnvironment().getObject()) instanceof FiniteStateAutomaton){
			FiniteStateAutomaton fsa = (FiniteStateAutomaton) this.getEnvironment().getObject();
			if(fsa.fromShowAnswer)
				title = "DFA Solution";
		}
		setTitle(title);
	}
...
//                  Get the suggested file name.
	...
	public boolean saveAsProblem(String problemDescription, boolean saveAs) {
		File file = saveAs ? null : environment.getFile();
        Codec codec = (Codec) environment.getEncoder();
        Serializable object = environment.getObject();
        if(environment.myObjects!=null  && environment.getActive()!=null && environment.getActive() instanceof EditorPane){
		...
		String filename = "";
				try{
					...
					count = new File(SaveAction.filePath);
					path = SaveAction.filePath;
